...
確度の小さい部分に誤検知が多く存在している。

検証データを用いて、99%の精度を担保できる閾値を決定した。
この閾値を元に、テストデータに対して予測を行った。
この予測群をクラスAとする。

以下にクラスAの予測について各モデルの予測したインデックスのベン図を示す。  
重複を見ると、3つのモデルが重複しているものは1484個である。
また、3，5，7層のモデルのみで予測しているものはそれぞれ976，133，816個である。
...
しかし階層化予測の重要性について、
階層化予測を用いることにより、深層学習の問題である誤検知を軽減し、高い精度での予測を行うことができる。

全体の精度としてSOTAは、97.31%%である。
これは、CNNとBiLSTMを組み合わせたモデルである。

Transformerの適用事例として最大の精度を誇るConformerは、96%であった。
これは、Transformer層に畳み込み層を入れることで、CNNとTransformerを組み合わせた音声認識の分野からの応用モデルである。

いずれも、本研究の95.28%を上回る精度である。

しかし、階層化予測を行うことによって、クラスAの予測については99.6%の精度で予測できている。
これは、SOTAを上回る精度である。
<div style="page-break-before:always"></div>
