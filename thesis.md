...
## 1.2. 研究概要
この研究は、深層学習と人間行動識別のベンチマークであるWISDMデータセットを用いて人間の行動を認識し、その予測の確度を評価し、信頼性の高い階層化予測を行う。
...
モデルの訓練データに19217、検証データに824、テストデータに7413のデータを用いた。
...
このあたりの処理は、Transformerの処理に最適化させるために行っている。
